                ...
                if (!empty($Uname) && !empty($Number) && !empty($Email)) {
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            $sql = "UPDATE users SET username=?, serialnumber=?, gender=?, email=?,User_pwd=?, user_date=CURDATE(), device_uid=?, device_dep=?, add_card=1 WHERE id=?";
                            $result = mysqli_stmt_init($conn);
                            if (!mysqli_stmt_prepare($result, $sql)) {
                                echo "SQL_Error_select_Fingerprint";
                                exit();
                            } else {
                                mysqli_stmt_bind_param($result, "sssssssi", $Uname, $Number, $Gender, $Email, $Password, $dev_uid, $dev_name, $user_id);
                                mysqli_stmt_execute($result);
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            if (!empty($Uname) && !empty($Email)) {
                                ...
                                $sql = "UPDATE users SET username='" . $Uname . "', serialnumber='" . $Number . "', gender='" . $Gender . "', email='" . $Email . "', device_uid='" . $dev_uid . "', device_dep='" . $dev_name . "' WHERE serialnumber='" . $Number . "'";
                                if (mysqli_query($conn, $sql)) {
                                    $sql = "UPDATE users_logs SET username='" . $Uname . "' WHERE serialnumber='" . $Number . "'";
                                    if (mysqli_query($conn, $sql)) {
                                        echo "Success";
                                    }
                                ...
                                // }
                                // else{
                                //     mysqli_stmt_bind_param($result, "sdssssi", $Uname, $dev_uid );
                                //     mysqli_stmt_execute($result);
